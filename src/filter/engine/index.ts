...
// value-imports `charts/lib/filter-algebra` → (type) `filter/composables/useFilterDimensions` →
// `platform/stores/*`: a strongly-connected cycle spanning O-B4R (charts) + O-B9 (stores). Under the
...
// `compile` / `fromDimSpecs` / `explain` / `isIdentity` / `encodeFilter` / `parseFilter` /
